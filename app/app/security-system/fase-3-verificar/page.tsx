...
import { ShieldAlert, CheckCircle2, FileText, BadgeInfo, AlertTriangle, Download, Plus, X } from "lucide-react";
...
export default function Fase3Verificar() {
  ...
  const handleDownloadVulnLog = () => {
    downloadCsvFile(
      `seguridad-bitacora-vulneraciones-${new Date().toISOString().slice(0, 10)}.csv`,
      ["Fecha detección", "Título", "Severidad", "Fase", "Horas transcurridas", "Activos", "Titulares afectados", "Descripción"],
      vulneraciones.map((inc) => [
        inc.fechaDeteccion,
        inc.titulo,
        inc.severidad,
        inc.faseActual,
        inc.horasTranscurridas,
        inc.activosAfectadosIds.join(" | "),
        inc.titularesAfectadosEst ?? 0,
        inc.descripcion,
      ]),
    );
  };
    ...
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
         ...
         <div className="flex gap-3">
           <div className="px-4 py-2 rounded-xl bg-red-50 border border-red-100">
             <span className="text-xs font-semibold text-red-600 uppercase">No Implementados</span>
             <p className="text-xl font-semibold text-red-700">{noImplementados}</p>
           </div>
           <div className="px-4 py-2 rounded-xl bg-amber-50 border border-amber-100">
             <span className="text-xs font-semibold text-amber-600 uppercase">Sin Evaluar</span>
             <p className="text-xl font-semibold text-amber-700">{sinEvaluar}</p>
           </div>
           <div className="px-4 py-2 rounded-xl bg-slate-50 border border-slate-200">
             <span className="text-xs font-semibold text-slate-500 uppercase">Vuln. Abiertas</span>
             <p className="text-xl font-semibold text-slate-700">{vulneraciones.filter(v => v.faseActual !== "Cierre").length}</p>
           </div>
         </div>
      </header>
      ...
      <div className="flex gap-6 border-b border-slate-200">
        <button 
          ...
          className={`pb-4 text-sm font-semibold border-b-2 transition-colors ${activeTab === 'auditorias' ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
        >
          8A. Programas de Auditoría
        </button>
        <button 
          onClick={() => setActiveTab("incidentes")}
          className={`pb-4 text-sm font-semibold border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'incidentes' ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-800'}`}
        >
          8B. Vulneraciones / Incidentes
          {vulneraciones.length > 0 && <span className="flex items-center justify-center h-5 w-5 bg-red-100 text-red-700 text-[10px] rounded-full font-semibold">{vulneraciones.length}</span>}
        </button>
      </div>
      ...
      <div className="space-y-6">
        {activeTab === "auditorias" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center">
              <h3 className="font-semibold text-slate-900">Auditorías Programadas y Resultados</h3>
              <button onClick={() => setShowAudForm(!showAudForm)} className="text-sm font-semibold text-primary hover:text-primary/80 flex items-center gap-1">
            ...
            {showAudForm && (
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                <h4 className="font-semibold text-slate-800">Nueva Auditoría</h4>
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest block mb-1">Referencia / Nombre *</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest block mb-1">Fecha de ejecución *</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest block mb-1">Auditor</label>
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest block mb-1">Alcance</label>
                ...
                {auditorias.map((aud: SgsdpAuditoria) => (
                  <div key={aud.id} className="border-b border-slate-100 last:border-0 p-4 hover:bg-slate-50 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                      <h4 className="font-semibold text-slate-900">{aud.referencia}</h4>
                      <p className="text-sm text-slate-500 mt-1">
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase mr-2 ${aud.tipo === 'interna' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}`}>{aud.tipo}</span>
                    ...
                    <div className="flex items-center justify-end gap-6 w-full md:w-auto">
                      <div className="text-right">
                        <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block mb-1">Fecha</span>
                        <span className="text-sm font-semibold text-slate-700">{aud.fechaEjecucion}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block mb-1">Hallazgos</span>
                        <span className={`text-sm font-semibold ${aud.hallazgos && aud.hallazgos.length > 0 ? 'text-amber-600' : 'text-emerald-600'}`}>
                          {aud.hallazgos ? aud.hallazgos.length : 0}
                        </span>
                      </div>
                      <div className="text-right">
                      <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block mb-1">Estado</span>
                      <span className={`px-2 py-0.5 rounded text-[10px] font-semibold uppercase tracking-widest border ${
            ...
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-5 flex gap-4">
               ...
               <div>
                 <h4 className="font-semibold text-indigo-900 text-sm">Validación del SGSDP</h4>
        ...
        {activeTab === "incidentes" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
             <div className="flex justify-between items-center">
              <h3 className="font-semibold text-slate-900">Bitácora de Vulneraciones (Art. 63 RLFPDPPP)</h3>
              <div className="flex items-center gap-2">
                <button onClick={handleDownloadVulnLog} className="text-sm font-semibold text-slate-700 hover:text-slate-900 flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-2">
                  <Download className="h-4 w-4" /> Descargar bitácora
                </button>
                <button onClick={() => setShowVulnForm(!showVulnForm)} className="text-sm font-semibold text-primary hover:text-primary/80 flex items-center gap-1">
            ...
            {showVulnForm && (
              <div className="bg-white rounded-2xl border border-red-200 shadow-sm p-6 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                <h4 className="font-semibold text-slate-800 flex items-center gap-2"><ShieldAlert className="h-5 w-5 text-red-500" /> Registrar Nueva Vulneración</h4>
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest block mb-1">Severidad</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest block mb-1">Activos Afectados</label>
                  ...
                  <div>
                    <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest block mb-1">Titulares Afectados (est.)</label>
                ...
                {(newVuln.severidad === "critica" || newVuln.severidad === "alta") && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700 flex items-start gap-2">
                    ...
                    <span><span className="font-semibold">Art. 40 LFPDPPP:</span> Las vulneraciones críticas/altas generan automáticamente una acción correctiva (CAPA) y deben notificarse a titulares. Plazo recomendado: 72 horas.</span>
                  </div>
                )}
                <div className="flex gap-3 justify-end">
                  ...
                  <div key={inc.id} className="border-b border-slate-100 last:border-0 p-4 hover:bg-slate-50 transition-colors flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <h4 className="font-semibold text-slate-900">{inc.titulo || inc.descripcion.substring(0, 30)}</h4>
                    ...
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block mb-1">Detectada</span>
                        <span className="text-sm font-semibold text-slate-700">{inc.fechaDeteccion}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block mb-1">SLA</span>
                        <span className={`text-sm font-semibold ${inc.horasTranscurridas > 72 ? 'text-red-600' : 'text-slate-900'}`}>{inc.horasTranscurridas}h</span>
                      </div>
                      <div className="text-right">
                        <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 block mb-1">Fase</span>
                        <select 
                          value={inc.faseActual}
                          onChange={(e) => moveVulneracionFase(inc.id, e.target.value as SgsdpVulneracion["faseActual"])}
                          className="text-xs font-semibold text-slate-900 bg-transparent border border-slate-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-primary"
                        >
